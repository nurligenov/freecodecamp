...
echo "$($PSQL "select GREATEST(MAX(opponent_goals), MAX(winner_goals)) from games;")"
...
echo "$($PSQL "select name from teams where team_id in (SELECT winner_id AS player_id FROM games where year = 2014 AND round = 'Eighth-Final' UNION ALL SELECT opponent_id AS player_id FROM games where year = 2014 AND round = 'Eighth-Final') order by name;")"
